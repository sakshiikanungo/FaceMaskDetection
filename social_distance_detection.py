...
def detect_and_predict_mask(frame, faceNet, maskNet):
    ...
    if len(faces) > 0:
        # stack the faces into a single numpy array to form a batch
        faces = np.vstack(faces)  # or np.concatenate(faces, axis=0)
        
        # make predictions on all faces at once
...
arg.add_argument("-m1", "--model1", type=str, default="mask_detector.keras", help="path to trained face mask detector model")
